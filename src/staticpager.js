...
(function ($) {
	...
	if (!Array.prototype.indexOf) {
		...
		$.pager = function (el, options) {
				...
				window.pager = [];
				savePoint = { "id" : stamp, "p" : base };
				window.pager.push(savePoint);
		...
		var methods = {
				...
				destroy : function( ) {
					if (window.pager) {
						for (var p in window.pager) {
							if ($(this).is(window.pager[p].p.$el)) {
								window.pager[p].p.$el.destroy();
							}
						}
					}
				},
				...
				update : function( filter ) {
					if (window.pager) {
						for (var p in window.pager) {
							if ($(this).is(window.pager[p].p.$el)) {
								window.pager[p].p.$el.filter(filter);
							}
						}
					}
				}
